...
describe Camp do
  ...
  it "has a name" do
    camp = FactoryBot.build(:camp, name: nil)
    expect(camp).not_to be_valid
  end

  it "has a location" do
    camp = FactoryBot.build(:camp, location: nil)
    expect(camp).not_to be_valid
  end

  it "has a start date at some point in the future" do
    camp = FactoryBot.build(:camp, start_date: 1.week.ago)
    expect(camp).not_to be_valid
  end

  it "has an end date on or after the start date" do
    #Probably want to refactor this into two tests.
    camp1 = FactoryBot.build(:camp, start_date: 2.weeks.since, end_date: 1.week.since)
    camp2 = FactoryBot.build(:camp, start_date: 4.weeks.since, end_date: 4.weeks.since)
    expect(camp1).not_to be_valid
    expect(camp2).to be_valid
  end

  it "has an application start date before the camp start date" do
    camp = FactoryBot.build(:camp, start_date: 2.weeks.since, app_start_date: 2.weeks.since)
    expect(camp).not_to be_valid
  end

  it "has an application end date before the camp start date" do
    camp = FactoryBot.build(:camp, start_date: 2.weeks.since, app_end_date: 2.weeks.since)
    expect(camp).not_to be_valid
  end

  it "has an application end date on or after the application start date" do
    camp = FactoryBot.build(:camp, app_start_date: 1.week.ago, app_end_date: 2.weeks.ago)
    expect(camp).not_to be_valid
  end
